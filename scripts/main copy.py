...
# PCAP Files
def save_logfiles(experiment_variables, triplets_list):
    '''Save the data logged while one testrun'''
    # Create a directory to store the logs
    timestamp = time.strftime("%Y%m%d_%H%M%S")
    host=experiment_variables["Host"]
    log_dir = f"logs/{timestamp}_{host}"
...
def main():
    ...
    for target_host, target_port in hosts:
        ...
        # Inititialise dictionaries and lists for logging
        status_code_count = {}
        request_data_list = []
